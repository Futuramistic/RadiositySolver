...
//static const char outputModelFilename[] = "new_model.out";
...
// Note that numPixelsOnWidth must be a even number.
{
    ...
    **********************************************************/
    double dA = 1.0 / pow(numPixelsOnWidth / 2.0, 2);
    for (int i = 0; i < numPixelsOnWidth; ++i){
        for (int j = 0; j < numPixelsOnWidth; ++j){
            double x = -1.0 + 1.0/numPixelsOnWidth + (double)i * 2.0 / numPixelsOnWidth;
            double y = -1.0 + 1.0/numPixelsOnWidth + (double)j * 2.0 / numPixelsOnWidth;
            deltaFormFactors[i * numPixelsOnWidth + j] = dA / (M_PI * pow(x*x + y*y + 1.0, 2));
        }
    }
}
...
// Note that numPixelsOnWidth must be a even number.
{
    ...
    **********************************************************/
    ...
    for (int i = 0; i < numPixelsOnWidth / 2; ++i){
        for (int j = 0; j < numPixelsOnWidth; ++j){
            double z = 1.0 / numPixelsOnWidth + (double)i * 2.0 / numPixelsOnWidth;
            double y = -1.0 + 1.0/numPixelsOnWidth + (double)j * 2.0 / numPixelsOnWidth;
            deltaFormFactors[i * numPixelsOnWidth + j] = dA * z / (M_PI * pow(y*y + z*z + 1.0, 2));
        }
    }
}
    ...
    // Need to set up the viewport, projection and view transfromation.
{
    ...
    float edge[3];
    VecCrossProd(lookUpVector, shooterQuad->normal, VecDiff(edge, shooterQuad->v[0], shooterQuad->v[1]));
    gluLookAt(
        shooterQuad->centroid[0], shooterQuad->centroid[1], shooterQuad->centroid[2],
    ...
    // and update the unshot power of their parent shooter quads.
{
    for (int i = 0; i < width * height; i++)
    {
        ...
        **********************************************************/
        ...
        for (int j = 0; j < 3; ++j)
        {
            float power = m->gatherers[g]->surface->reflectivity[j] * shotPower[j] * deltaFormFactors[i] / m->gatherers[g]->area;
            m->gatherers[g]->radiosity[j] += power;
            m->gatherers[g]->shooter->unshotPower[j] += power * m->gatherers[g]->area;
        }
    }
}
